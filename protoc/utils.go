...
func BuildProtoParams(path string, rootDir files.RootDir, protoOutDir string, pkg string) (out ProtoParams, err error) {
	if protoOutDir == "" {
	...
	// github.com/foo/bar
	importPath, err := files.GetImportPath(rootDir.String())
	if err != nil {
