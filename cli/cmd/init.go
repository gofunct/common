...
func newInitCommand(c *cli.Ctx) *cobra.Command {
	var (
		skipViper bool
	)
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			entries := []*entry{
				...
				{Path: root.Join("variables.tf").String(), Template: templates.TfVariableTemplate()},
				{Path: root.Join("main.tf").String(), Template: templates.TfMainTemplate()},
				{Path: root.Join("output.tf").String(), Template: templates.TfOutputTemplate()},
